...
const columns: Column<SubscriptionDisplay>[] = [
...
export default function Contact() {
    ...
    // DataTable states
    const [rows, setRows] = useState<SubscriptionDisplay[]>([]);
    ...
    const fetchData = useCallback(async (query: DataTableQuery) => {
        ...
        try {
            ...
            const json: { data: SubscriptionRow[]; meta: PaginationMeta } = await res.json();

            const subscriptionData: SubscriptionDisplay[] = json.data.map((item) => ({
                id: item.id,
                email: <p className="line-clamp-2">{item.email}</p>,
            }));

            setRows(subscriptionData);
            setMeta(json.meta);
        } catch (error) {
            console.error(error);
        } finally {
